...
import edu.pe.vallegrande.maestro2.db.service.imple.CrudDatosBajaService;


@WebServlet("/DescargarDatos")
public class DatBajaController extends HttpServlet {
    ...
    private void exportPDF(OutputStream out) throws IOException {
        ...
        try {
            ...
            document.open();
            document.add(new Paragraph("Informe de Bienes BAJOS del Area de Contabilidad"));

            // Agrega el contenido real del informe
            new CrudDatosBajaService().getPDFData(document);
    ...
    private void exportCSV(OutputStream out) {
        ...
        try {
            String csvData = new CrudDatosBajaService().getCSVData();
    ...
    private void exportXLS(OutputStream out) {
        try {
            HSSFWorkbook workbook = new CrudDatosBajaService().getXLSData();
            workbook.write(out);
        } catch (IOException e) {
            e.printStackTrace(); 
        }
